...
class DynaControl:
    ...
    def __init__(self):
        ...
        rospy.Subscriber("/cmd_vel/dyna/multiple", DynaTwistMultiple, self.multiple_commands_callback)  
        rospy.loginfo("Successfully subscribed to /cmd_vel/dyna and /cmd_vel/dyna/multiple. \n Waiting for commands.")
    ...
    def gotodegree(self, data, multiple = False):
        # if data.joint == 'reset': # if the command is to reset the dynamixel units to their initial position
        ...
        if not multiple:
            ...
            # rospy.loginfo('Moving to position: '+ str(goal_position))
        ...
        if multiple: # if the command is a multiple command (a list of commands) to move multiple joints at the same time
            rospy.loginfo("recieved a multiple movement command")
            commands = data.commands
            safety_checked_commands = []
            i = 0
            for command in commands:
                speed = int(command.speed.angular.x)
                dposition = int(command.position)
                degrees=True
                req_joint = command.joint

                goal_position = dposition
                ...
                safety_checked_commands.append([req_joint])
                ...
                safety_checked_commands[i].append([goal_position,speed,degrees])
                i+=1
            

            for i in range(len(safety_checked_commands)):
                rospy.loginfo('Moving dynamixel ID '+safety_checked_commands[i][0]+
                              'to position: '+ str(safety_checked_commands[i][1][0])
                              +' with speed: '+str(safety_checked_commands[i][1][1]))
                # Go to the specified position for specific dynamixel id in degrees with specified speed
                # self.serial_connection.goto(safety_checked_commands[i][0], 
                #                             safety_checked_commands[i][1][0],
                #                             speed=safety_checked_commands[i][1][1],
                #                             degrees=degrees)
                
            rospy.loginfo('Waiting for the group movement to finish...')
