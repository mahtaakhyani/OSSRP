...
class GazeDetector:
    def __init__(self, video_path, iris_diameter_mm, focal_length_mm):
        ...
        self.data_file = f'{self.dir}/extracted_data_gaze.xlsx'
    ...
    def analyze(self, camera=True):
        ...
        data_headers = ['Time', 'dT', 'Gaze', 'Head']
        data = []
            ...
            while True:
                try:
                    ...
                    if results.multi_face_landmarks:
                        ...
                        right_corner_bottom = mesh_points[450]

                        if frame_number == 1:
                            first_dist_right = np.linalg.norm(right - center_right)
                            first_ratio = first_dist_right / r_radius
                            first_dist_right_up = np.linalg.norm(right - right_corner_bottom)
                            first_ratio_up = first_dist_right_up / r_radius

                        new_ratio = (np.linalg.norm(right - center_right) / r_radius)
                        new_normal_dist = new_ratio - first_ratio

                        new_ratio_up = np.linalg.norm(right - right_corner_bottom) - first_dist_right_up
                        new_normal_dist_up = new_ratio_up / r_radius

                        if (rc_radius / r_radius) < new_normal_dist:
                            gaze_position = 'left'
                        elif new_normal_dist < 0 - (rc_radius / r_radius):
                            gaze_position = 'right'
                        else:
                            gaze_position = 'center'

                        if 0.01 < new_normal_dist_up:
                            gaze_position += ' up'
                        elif new_normal_dist_up < -0.025:
                            gaze_position += ' down'
                        else:
                            gaze_position += ' center'

                        cv2.line(frame, right_corner_bottom, right, (255, 0, 0), 2)
                        cv2.circle(frame, right_corner_bottom, 4, (0, 255, 0))
                        ...
                        nose_hip = mesh_points[6]
                        chin_tip = mesh_points[18]
                        chin_bot = mesh_points[199]
                        chick_center = mesh_points[36]
                        chick_left = mesh_points[137]

                        if frame_number == 1:
                            nose_len = np.linalg.norm(nose_hip - nose_tip)
                            chin_len = np.linalg.norm(chin_tip - chin_bot)
                            chick_len = np.linalg.norm(chick_left - chick_center)

                        cur_nose_len = np.linalg.norm(nose_hip - nose_tip)
                        cur_chin_len = np.linalg.norm(chin_tip - chin_bot)
                        cur_chick_len = np.linalg.norm(chick_left - chick_center)

                        if cur_nose_len < nose_len:
                            orientation = "Facing Up"
                        elif cur_chin_len < chin_len:
                            orientation = "Facing Down"
                        else:
                            orientation = "Facing center"

                        if cur_chick_len < chick_len - 2:
                            orientation += ", left"
                        elif cur_chick_len > chick_len + 2:
                            orientation += ", right"
                        else:
                            orientation += ", center"
                        ...
                        if frame_number % 30 == 0:
                            data.append([current_time, real_frame_number, gaze_position, orientation])
...
if __name__ == '__main__':
    ...
    gaze_detector.analyze(camera=False)
