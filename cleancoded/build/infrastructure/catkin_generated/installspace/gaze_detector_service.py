...
class GazeTracking:
    ...
    def gazedirection(self, data):
        ...
        self.frame= self.convert_frame(data.frame)
    ...
    def head_position(self):
        ...
        cv2.line(self.frame, p1, p2, (0, 255, 255), 2)
        cv2.line(self.frame, tuple(x1), tuple(x2), (255, 255, 0), 2)
        cv2.putText(self.frame, str(p1), p1, font, 1, (0, 255, 255), 1)

        cv2.putText(self.frame, str(self.ang1), tuple(
         p1), font, 2, (128, 255, 255), 3)
        cv2.putText(self.frame, str(self.ang2), tuple(
    ...
    def head_pose_points(self):
        ...
        rear_size = 1
        rear_depth = 0
        front_size = self.frame.shape[1]
        front_depth = front_size*2
        val = [rear_size, rear_depth, front_size, front_depth]
        point_2d = self.get_2d_points(self.frame, self.camera_matrix, val)
    ...
    def head_rpy(self):
        ...
        if self.ang1 in range(-40, 40) and self.ang2 in range(-40, 40):
            ...
            cv2.putText(self.frame, str(head_pos_str), (30, 30), font, 2, (255, 255, 128), 3)
    ...
    def convert_frame(self, data, cv2window=False): # convert the frame to a numpy array from ROS image
        try:
            ...
            arr = np.array(self.modif_image, dtype=np.uint8).reshape((640,480,3))
            return arr
    ...
    def draw_annotation_box(self,
       ...
       point_2d = self.get_2d_points(self.frame, self.camera_matrix, val)
       # # Draw all the lines
       # cv2.polylines(self.frame, [point_2d], True, color, line_width, cv2.LINE_AA) # draw the box around the face
       cv2.line(self.frame, tuple(point_2d[1]), tuple(
         point_2d[6]), color, line_width, cv2.LINE_AA) # draw line from left ear to right ear
       cv2.line(self.frame, tuple(point_2d[2]), tuple(
         point_2d[7]), color, line_width, cv2.LINE_AA) # draw line from left eye to right eye
       cv2.line(self.frame, tuple(point_2d[3]), tuple(
