...
class MotorsController():
    ...
    def hand_controller(self,data):
        if data.motor == 0:
            if self.args.__getattribute__('lhand'): dynamixel_id = self.args.__getattribute__('lhand')
            else: dynamixel_id = 5
        if data.motor == 1:
            if self.args.__getattribute__('rhand'): dynamixel_id = self.args.__getattribute__('rhand')
            else: dynamixel_id = 6
    ...
    def head_controller(self,data):
        if data.motor == 0:
                    if self.args.__getattribute__('head'): dynamixel_id = self.args.__getattribute__('head')
                    else: dynamixel_id = 7
        if data.motor == 1:
                    if self.args.__getattribute__('neck'): dynamixel_id = self.args.__getattribute__('neck')
                    else: dynamixel_id = 4
